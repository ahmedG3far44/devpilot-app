...
type ProjectType = "react" | "next" | "express" | "nest" | "static"
type ProjectStatus = "active" | "failed" | "stopped"
type DeploymentStatus = "success" | "failed";
...
function ProjectConfiguration({ projectData }: { projectData: ProjectData }) {
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2">
                                ...
                                <div key={envVar.id} className="rounded-lg border border-border bg-muted/30 p-3 space-y-2">
                                        ...
                                        <div className="flex gap-1">
                                            {editingEnvId === envVar.id ? (
                                                <>
                                                    ...
                                                    <Button
                                                        onClick={() => cancelEnvEdit(envVar.id)}
                                                        size="sm"
                                                        variant="ghost"
                                                        className="h-7 w-7 p-0"
                                                    >
                                                ...
                                                <>
                                                    ...
                                                    <Button
                                                        onClick={() => startEditEnv(envVar.id)}
                                                        size="sm"
                                                        variant="ghost"
                                                        className="h-7 w-7 p-0"
                                                    >
                                                    ...
                                                    <Button
                                                        onClick={() => deleteEnvVariable(envVar.id)}
                                                        size="sm"
                                                        variant="ghost"
                                                        className="h-7 w-7 p-0"
                                                    >
                                    ...
                                    {editingEnvId === envVar.id ? (
                                        ...
                                        <p className="text-xs font-mono break-all text-muted-foreground">
                                            {isSensitive(envVar.key) && !maskedValues[envVar.id]
                                                ? "•".repeat(20)
                                                : envVar.value}
                                        </p>
                                    )}
                    ...
                    <div className="space-y-2 font-mono text-sm">
                        {logs.map((log) => (
                            <div key={log.id} className="flex gap-3 rounded border border-border bg-muted/30 p-2">
                                ...
                                <span className={cn("font-semibold uppercase", getLogLevelColor(log.level))}>
                                    [{log.level}]
                                </span>
